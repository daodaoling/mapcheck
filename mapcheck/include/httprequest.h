#ifndef HTTPR_REQUEST_H
#define HTTPR_REQUEST_H
#include<iostream>
#include <curl/curl.h>
#include <curl/easy.h>
using namespace std;
...
class HttpRequest{
    ...
    public :
        static CURLcode send_post(const string &url, const string &postParams);
...
//http POST请求  
CURLcode HttpRequest::send_post(const string &url, const string &postParams)
{
    string response;
	// curl初始化  
	...
	if (curl)
	{
		...
		//write_log("before curl_easy_perform !");
		// 开启post请求
		res = curl_easy_perform(curl);
		//write_log("after curl_easy_perform !");
        //std::cout<<"after"<<endl;
	}
	//释放curl 
	curl_easy_cleanup(curl);
	//write_log("before return !");
	return res;
}
#endif